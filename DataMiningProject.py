...
if uploaded_file is not None:
    ...
    st.write("""
    #### Description
    Visualizing data helps in understanding the distribution and relationships between features. This section provides options for creating histograms and box plots.
    """)

    plot_type = st.selectbox("Select plot type", ["Histogram", "Box Plot"])

    if plot_type == "Histogram":
        column = st.selectbox("Select column for histogram", df_normalized.select_dtypes(include=[np.number]).columns)
        plt.figure(figsize=(10, 4))
        sns.histplot(df_normalized[column], kde=True)
        st.pyplot(plt)
    elif plot_type == "Box Plot":
        column = st.selectbox("Select column for box plot", df_normalized.select_dtypes(include=[np.number]).columns)
        plt.figure(figsize=(10, 4))
        sns.boxplot(df_normalized[column])
        st.pyplot(plt)
